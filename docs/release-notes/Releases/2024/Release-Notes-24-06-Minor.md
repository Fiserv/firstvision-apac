...
|    |                     | Changes                                                      | 
|    |                     | ------------------------------------------------------------ | 
|    |                     | - `paymentChangeDate`, `requestDays`, `suspenseStartDate`,    | 
|    |                     |   `suspenseEndDate`, and `customerName` added in request/     | 
|    |                     |   response                                                   | 
|    |                     | - `informationalMessage` added in response                   | 
|    |                     | - `maxLength` updated from 24 to 17 for                      | 
|    |                     |   `nominatedPaymentAmountOrPercentage` in request            | 
|    |                     | - Value '9' added in `paymentType` enum and its              | 
|    |                     |   description in response/request                            | 
